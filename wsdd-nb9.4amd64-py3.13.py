...
class MulticastHandler:
    ...
    def init_v4(self) -> None:
        ...
        # v4: member_request format differs by OS:
        #   Linux/FreeBSD: ip_mreqn = { mc_addr(4), intf_addr(4), ifindex(4) } = 12 bytes
        #   NetBSD/OpenBSD: ip_mreq = { mc_addr(4), intf_addr(4) } = 8 bytes (no ifindex)
        #   SunOS: ip_mreq without ifindex
        if platform.system() in ('SunOS', 'NetBSD', 'OpenBSD'):
...
RTA_IFA: int = 0x20
RTA_BRD: int = 0x80  # broadcast or point-to-point address

# from sys/socket.h
CTL_NET: int = 4
NET_RT_IFLIST: int = 3
NET_RT_IFLIST2: int = 5  # NetBSD amd64: uses if_msghdr2/ifa_msghdr2 format

# from sys/net/if.h
IFF_LOOPBACK: int = 0x8
# IFF_MULTICAST values differ per OS:
#   NetBSD: 0x8000  (0x800 is IFF_SIMPLEX on NetBSD)
#   OpenBSD: 0x8000
#   FreeBSD/Darwin: 0x800
if platform.system() in ('NetBSD', 'OpenBSD'):
    IFF_MULTICAST: int = 0x8000
else:
...
SA_ALIGNTO: int = ctypes.sizeof(ctypes.c_uint32) if platform.system() == "Darwin" else ctypes.sizeof(ctypes.c_long)
...
class RouteSocketAddressMonitor(NetworkAddressMonitor):
    ...
    # NetBSD uses the same header layout as FreeBSD/Darwin: { u_short msglen, u_char version,
    # u_char type, int addrs, int flags }
    # OpenBSD has an extended layout with additional fields.
    IF_COMMON_HDR_DEF = '@HBBii' if platform.system() != 'OpenBSD' else '@HBBHHHBBiii'

    # from net/if.h
    RTM_NEWADDR: int = 0xC
    RTM_DELADDR: int = 0xD
    # NetBSD amd64 uses RTM_IFINFO2=0x14; i386/other BSD use RTM_IFINFO=0x0E
    RTM_IFINFO: int = (0x14 if platform.system() == 'NetBSD' and ctypes.sizeof(ctypes.c_long) == 8
                       else 0xE)

    # from route.h: NetBSD uses 0x4, FreeBSD/Darwin/OpenBSD use 0x5
    RTM_VERSION: int = 0x4 if platform.system() == 'NetBSD' else 0x5

    # if_data layout appended after the common if_msghdr header.
    # The struct package does not allow to specify those, thus we hard code them as chars (x4).
    IF_MSG_DEFS: Dict[str, str] = {
        # if_data in if_msghdr is prepended with an u_short _ifm_spare1, thus the 'H' at the beginning
        ...
        'OpenBSD': '4c3I13Q1Iql',
        # NetBSD 9 (RTM_VERSION=4 compat wire format), arch-dependent:
        # 32-bit (sizeof long=4): if_msghdr=84, hH(spare)+4c+16xI(u_long counters)
        # 64-bit (sizeof long=8): if_msghdr=152, hH(spare)+4c+4x(pad)+14Q+2q(timeval)
        # Verified by hex dump and C sizeof on both i386 and amd64.
        'NetBSD': ('hH4c4x14Q2q' if ctypes.sizeof(ctypes.c_long) == 8 else 'hH4c16I'),
    }

    # ifa_msghdr suffix format per OS (appended to IF_COMMON_HDR_DEF):
    # NetBSD: wire format has IFA_MSGHDR_SIZE=24 on both i386 and amd64 (verified from buffer).
    # C sizeof(ifa_msghdr) is larger but the sysctl wire format is shorter.
    # FreeBSD/Darwin = @HBBii(12) + hi = 20 bytes
    # OpenBSD        = @HBBHHHBBiii(32) + nothing extra
    IFA_MSGHDR_DEFS: Dict[str, str] = {
        'FreeBSD': 'hi',
        'Darwin':  'hi',
        'OpenBSD': '',
        'NetBSD':  '3i',  # 24 bytes on both i386 and amd64 (wire format)
    }

    # from net/if.h (struct ifa_msghdr), per-OS suffix after IF_COMMON_HDR_DEF
    IFA_MSGHDR_DEF: str = IF_COMMON_HDR_DEF + IFA_MSGHDR_DEFS.get(platform.system(), 'hi')
    ...
    use_extended_hdr: bool = False  # True only for OpenBSD which has extra fields in common header
    ...
    def __init__(self, aio_loop: asyncio.AbstractEventLoop) -> None:
        ...
        self.IF_MSGHDR_SIZE = struct.calcsize(self.IF_COMMON_HDR_DEF + self.IF_MSG_DEFS[sys_name])
        self.is_openbsd = sys_name == 'OpenBSD'
        self.use_extended_hdr = sys_name == 'OpenBSD'  # only OpenBSD uses extended common header

        # NetBSD amd64 uses NET_RT_IFLIST2 (returns RTM_IFINFO2=0x14 + RTM_NEWADDR=0x0c).
        # NetBSD i386 and all other BSDs use NET_RT_IFLIST (returns RTM_IFINFO=0x0e + RTM_NEWADDR).
        self.net_rt_iflist = (NET_RT_IFLIST2
                              if sys_name == 'NetBSD' and ctypes.sizeof(ctypes.c_long) == 8
                              else NET_RT_IFLIST)

        logger.debug('RouteSocket struct sizes: IF_COMMON_HDR={} IF_MSGHDR={} IFA_MSGHDR={} SA_ALIGNTO={}'.format(
            struct.calcsize(self.IF_COMMON_HDR_DEF),
            self.IF_MSGHDR_SIZE,
            self.IFA_MSGHDR_SIZE,
            SA_ALIGNTO))

    def do_enumerate(self) -> None:
        super().do_enumerate()
        mib = [CTL_NET, socket.AF_ROUTE, 0, 0, self.net_rt_iflist, 0]
        rt_mib = (ctypes.c_int * len(mib))()
        rt_mib[:] = [ctypes.c_int(m) for m in mib]

        libc = ctypes.CDLL(ctypes.util.find_library('c'), use_errno=True)

        # Set argtypes explicitly so ctypes marshals 64-bit pointers and size_t correctly.
        # Without this, on amd64 ctypes defaults to int (4-byte) for all args -> EINVAL.
        libc.sysctl.restype = ctypes.c_int
        libc.sysctl.argtypes = [
            ctypes.POINTER(ctypes.c_int),       # const int *name
            ctypes.c_uint,                       # u_int namelen
            ctypes.c_void_p,                     # void *oldp
            ctypes.POINTER(ctypes.c_size_t),     # size_t *oldlenp
            ctypes.c_void_p,                     # const void *newp
            ctypes.c_size_t,                     # size_t newlen
        ]

        # Ask kernel for routing table size first (oldp=None for size query).
        rt_size = ctypes.c_size_t()
        rt_mib_ptr = ctypes.cast(rt_mib, ctypes.POINTER(ctypes.c_int))
        if libc.sysctl(rt_mib_ptr, ctypes.c_uint(len(rt_mib)),
                       None, ctypes.byref(rt_size),
        ...
        if libc.sysctl(rt_mib_ptr, ctypes.c_uint(len(rt_mib)),
                       ctypes.cast(rt_buf, ctypes.c_void_p), ctypes.byref(rt_size),
                       None, ctypes.c_size_t(0)):
            raise OSError(ctypes.get_errno(), os.strerror(ctypes.get_errno()))
...
def parse_args() -> None:
    ...
    if args.verbose == 1:
        ...
        # asyncio.get_event_loop() is deprecated in Python 3.10+; use get_event_loop_policy()
    ...
    if not args.uuid:
        ...
        # machine uuid: try machine-id file first but also check for hostid (FreeBSD/NetBSD)
        args.uuid = read_uuid_from_file('/etc/machine-id') or \
            read_uuid_from_file('/etc/hostid') or \
            read_uuid_from_file('/etc/machine-uuid') or \
...
def create_address_monitor(system: str, aio_loop: asyncio.AbstractEventLoop) -> NetworkAddressMonitor:
    ...
    elif system in ['FreeBSD', 'Darwin', 'OpenBSD', 'NetBSD']:
